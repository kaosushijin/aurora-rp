...
# Resource loss analysis system (unchanged - still using semantic analysis)
...
# ------------------ Main Loop ------------------ #
async def main_loop():
    print_wrapped("Aurora RPG Client (TN4 - Pure Semantic Memory)", color=Fore.CYAN)
    print_wrapped("Enhanced with LLM-driven content categorization and intelligent preservation.", color=Fore.CYAN)
    ...
    if DEBUG:
        print(Fore.MAGENTA + "[Debug] Pure Semantic Memory System Active:")
    ...
    while True:
        ...
        # Enhanced Memory Management - Use new pure semantic condensation system
        ...
        if total_memory_tokens > MAX_MEMORY_TOKENS:
            if DEBUG:
                ...
                print(Fore.MAGENTA + "[Debug] Using pure semantic condensation system (no keyword triggers)...")
            ...
            if DEBUG:
                ...
                # Show recent condensation types if available
                recent_condensations = [mem for mem in memories[-10:] 
                                      if mem.get("condensation_type")]
                if recent_condensations:
                    condensation_summary = {}
                    for mem in recent_condensations:
                        c_type = mem.get("condensation_type", "unknown")
                        condensation_summary[c_type] = condensation_summary.get(c_type, 0) + 1
                    
                    print(Fore.MAGENTA + f"[Debug] Recent semantic categorizations: {dict(condensation_summary)}")
        ...
        # Enhanced debug final message composition
        if DEBUG:
            ...
            # Show memory composition by type
            memory_by_type = {"user": 0, "assistant": 0, "system": 0}
            condensed_by_type = {"story_critical": 0, "character_development": 0, "world_building": 0, "interaction": 0, "fallback": 0}
            
            for mem in memory_messages:
                role = mem.get("role", "unknown")
                if role in memory_by_type:
                    memory_by_type[role] += 1
                
                # Track condensed memory types
                if role == "system" and "condensation_type" in str(mem):
                    # This is a rough way to detect condensed memories - could be improved
                    condensed_by_type["story_critical"] += 1
            
            print(Fore.CYAN + f"[Debug] Memory composition: {dict(memory_by_type)}")
