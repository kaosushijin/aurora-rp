...
def load_memory() -> List[Dict[str, Any]]:
    if SAVE_FILE.exists():
        ...
        if not memories:
            # add system prompt if memory exists but empty
...
# ------------------ Main Loop ------------------ #
async def main_loop():
    print(Fore.CYAN + "Aurora RPG Client (POC)")
    print(Fore.CYAN + "Type your action. Press ENTER twice to submit.")
    print(Fore.CYAN + "Commands: /mems /dump /quit\n")
    ...
    while True:
        ...
        if raw_input_text.startswith("/quit"):
            ...
            break
        elif raw_input_text.startswith("/mems"):
            for mem in memories[-10:]:
                print(Fore.YELLOW + f"{mem['timestamp']} [{mem['role']}]: {mem['content']}")
            continue
        elif raw_input_text.startswith("/dump"):
            if DEBUG:
                print(json.dumps(memories, indent=2))
            else:
                print(Fore.YELLOW + "[System] --debug not enabled. Use --debug to view full memory.")
            continue
