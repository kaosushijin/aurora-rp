...
def prune_memory(memories: List[Dict[str, Any]], max_tokens: int) -> List[Dict[str, Any]]:
    ...
    while total_tokens > max_tokens:
        # Remove or summarize older memories
        memories.pop(0)  # Remove the oldest memory
        total_tokens = sum(estimate_tokens(mem["content"]) for mem in memories)
    return memories
...
async def main_loop():
    ...
    while True:
        ...
        # Prune memory to stay within token limit
        memories = prune_memory(memories, MAX_MEMORY_TOKENS)
