...
async def condense_memory(memories, fraction=0.2):
    """Updated memory condensation respecting new token limits"""
    if not memories:
        return memories

    chunk_size = max(1, int(len(memories) * fraction))
    chunk, memories = memories[:chunk_size], memories[chunk_size:]

    text = "\n\n".join(f"{m['role'].capitalize()}: {m['content']}" for m in chunk)

    # Ensure condensation prompt fits within tension system budget
    prompt = f"Condense memories while preserving story:\n{text}"
    if estimate_tokens(prompt) > TENSION_SYSTEM_TOKENS:
        # If too large, condense more aggressively
...
# ------------------ Main Loop ------------------ #
async def main_loop():
    print_wrapped("Aurora RPG Client (TN2 - Optimized Token Budget)", color=Fore.CYAN)
    ...
    while True:
        ...
        # NEW: Validate input length before processing
        ...
        # Memory condensation with updated token limits
        ...
        if DEBUG:
            ...
            print(Fore.YELLOW + f"  System prompts: {sum(estimate_tokens(msg['content']) for msg in system_messages)} tokens")
            print(Fore.YELLOW + f"  Memory messages: {sum(estimate_tokens(msg['content']) for msg in memory_messages)} tokens")
            print(Fore.YELLOW + f"  User input: {estimate_tokens(raw_input_text)} tokens")
