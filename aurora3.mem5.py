...
# ------------------ Utility: Word Wrap (preserve paragraphs) ------------------ #
def print_wrapped(text: str, color=Fore.GREEN, indent: int = 0):
    ...
    Each paragraph (separated by blank lines) is wrapped individually.
    """
    width = get_terminal_width() - indent
    wrapper = textwrap.TextWrapper(width=width, subsequent_indent=' ' * indent)

    # Split on double newlines (paragraphs), or single newline if paragraph breaks are used
    paragraphs = text.split("\n\n")
    for i, para in enumerate(paragraphs):
        lines = para.splitlines()
        # Wrap each original line in paragraph to preserve intentional single line breaks
...
async def main_loop():
    ...
    while True:
        ...
        # Echo user input neatly wrapped
        ...
        # Prepend system prompts dynamically
        system_messages = [
